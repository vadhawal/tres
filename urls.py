...
from userProfile.views import contact_us, suggest_store, facebook_view, getShopTalk, getUserReviews, followObject, unfollowObject, followWish, unfollowWish, getTrendingStores, getTrendingDeals, getTrendingReviews, render_wish, get_reldata, get_reviews_by_user
...
urlpatterns = patterns("",
    ...
    url(r'^review/rating/(?P<object_id>\d+)/(?P<direction>up|down|clear)/vote/?$', vote_on_object, review_rating_dict, name="rating_on_reviews"),
    url(r'^album/(?P<object_id>\d+)/(?P<direction>up|down|clear)/vote/?$', vote_on_object, album_dict, name="vote_on_albums"),
    url(r'^image/(?P<object_id>\d+)/(?P<direction>up|down|clear)/vote/?$', vote_on_object, image_dict, name="vote_on_images"),
    ...
    url(r'^vote/object/(?P<content_type_id>\d+)/(?P<object_id>\d+)/users/$', 'voting.views.get_voters_info', name='get_voters_info'), 
    url(r'^vote/object/(?P<content_type_id>\d+)/(?P<object_id>\d+)/users/range/(?P<sIndex>\d+)/(?P<lIndex>\d+)/$', 'voting.views.get_voters_info_inc', name='get_voters_info_inc'), 
    ...
    url(r'^view/wish/(?P<wish_id>[\d]+)/$', view_wish, name='view_wish'),
    url(r'^view/deal/(?P<deal_id>[\d]+)/$', view_deal, name='view_deal'),
    url(r'^view/post/(?P<post_id>[\d]+)/$', view_post, name='view_post'),
    url('^', include('follow.urls')),
    url(r'^wish/users/(?P<content_type_id>\d+)/(?P<object_id>\d+)/range/(?P<sIndex>\d+)/(?P<lIndex>\d+)/$',
        get_wishlist, name='get_wishlist'),
    url(r'^deal/store/(?P<content_type_id>\d+)/(?P<object_id>\d+)/range/(?P<sIndex>\d+)/(?P<lIndex>\d+)/$',
        get_deallist, name='get_deallist'),
    url(r'^deal/filter/(?P<store_id>\d+)/(?P<sub_category>[%&-_ \w\d]+)/range/(?P<sIndex>\d+)/(?P<lIndex>\d+)/$',
        get_filtered_deallist, name='get_filtered_deallist'),
    url(r'^relatedstores/(?P<store_id>\d+)/filter/(?P<sub_category>[%&-_ \w\d]+)/range/(?P<sIndex>\d+)/(?P<lIndex>\d+)/$',
        get_related_stores, name='get_related_stores'),
    url(r'^wish/(?P<wish_id>\d+)/share/$', shareWish, name='shareWish'),
    url(r'^deal/(?P<deal_id>\d+)/share/$', shareDeal, name='shareDeal'),
    url(r'^store/(?P<store_id>\d+)/share/$', shareStore, name='shareStore'),
    ...
    url(r'^wish/(?P<wish_id>\d+)/unfollow/$', unfollowWish, name='unfollowWish'),
    url(r'^object/(?P<content_type_id>\d+)/(?P<object_id>\d+)/follow/$', followObject, name='followObject'),
    url(r'^unfollow/object/(?P<content_type_id>\d+)/(?P<object_id>\d+)/$', unfollowObject, name='unfollowObject'), 
    url(r'^(?P<category_slug>[-\w\d]+)/subcategories/$', blog_subcategories , name='blog_subcategories'),
    url(r'^stores/(?P<parent_category_slug>[%&-_ \w\d]+)/(?P<sub_category_slug>[%&-_ \w\d]+)/$', get_vendors, name="get_vendors" ),
    url(r'^stores/[%&-_ \w\d]+/$', get_vendors_allsub, name="get_vendors_allsub" ),
    url(r'^stores/$', get_vendors_all, name="get_vendors_all" ),
    url(r'^user/(?P<username>[%&-_ \w\d]+)/profile/image$', get_profile_image, name="get_profile_image" ),
    ...
    url(r'^reviews/user/(?P<user_id>\d+)/$', get_reviews_by_user, name='get_reviews_by_user'),  
    url(r'^store/(?P<blogpost_id>[\d]+)/logo/edit/$', edit_blog_image, name='edit_blog_image'),  
    url(r'^admin/django-ses/', include('django_ses.urls')),
    url(r'^user/(?P<user_id>\d+)/reviews/range/(?P<sIndex>\d+)/(?P<lIndex>\d+)/$', getUserReviews , name='getUserReviews'),
